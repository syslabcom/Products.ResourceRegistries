...
class TestStylesheetCaching(RegistryTestCase):

    def afterSetUp(self):
        self.tool = getattr(self.portal, CSSTOOLNAME)
        self.tool.clearResources()

    def testCacheFill(self):
        ...
        # do we have a cache? (we store each individual item and the compiled one)
        self.assertEqual(len(self.tool._cached_resources.keys()),4)
        
        
    def testCacheAfterChange(self):
        """test what happens if we unregister a stylesheet"""
        self.tool.registerStylesheet('ham')
        self.tool.registerStylesheet('spam')
        self.tool.registerStylesheet('eggs')
        
        old_cookid = self.tool.cookedresources[0].getId()
        self.failUnless(old_cookid in self.tool._cached_resources.keys())

        self.tool.registerStylesheet("bread")
        self.assertEqual(len(self.tool._cached_resources.keys()),5)
        self.assertEqual(len(self.tool.cookedresources), 1)

        cookid = self.tool.cookedresources[0].getId()
        self.failUnless(cookid in self.tool._cached_resources.keys())
        self.failIf(old_cookid in self.tool._cached_resources.keys())
        
    def testDebugMode(self):
        """test if the cache is used if debug mode is enabled"""
        ...
        self.assertEqual(len(self.tool.concatenatedresources.keys()), 3)
        # do we have a cache? (we store each individual item and the compiled one)
        self.assertEqual(len(self.tool._cached_resources.keys()),0)

    def testCacheDisabled(self):
        self.tool.registerStylesheet('ham',doCook=False)
        self.tool.registerStylesheet('spam',doCook=False)
        self.tool.registerStylesheet('eggs',doCook=False)
        # shouldn't be something in there yet
        self.assertEqual(len(self.tool._cached_resources.keys()),0)
        # now cook it
...
def test_suite():
    ...
    suite = TestSuite()
    suite.addTest(makeSuite(TestStylesheetCaching))
